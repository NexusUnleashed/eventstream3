export default class EventStream extends EventTarget {
    ...
    registerEvent(event, callback, once = false, ) {
        ...
        this.addEventListener(event, async ({detail})=>{listener.callback(detail)}, {
            once: once,
    ...
    removeListener(event, listener) {
        ...
        if (typeof streamEvent === 'undefined') {
            console.log(`eventStream: Attempted to remove event that does not exist ${event}`)
            return;
        }
        ...
        if (typeof listener === 'string') {
            ...
            if (i >= 0) { this.removeListener(event, i) }
        }
    }
...
/* purge eventStream
for (const ev in eventStream.stream) {
  for (const cb of eventStream.stream[ev]) {
    eventStream.removeListener(ev, cb.callback)
  }
}
*/